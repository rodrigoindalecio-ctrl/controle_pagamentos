...
async function syncBalances() {
    console.log('--- INICIANDO AUDITORIA GLOBAL DE SALDOS (v3 - Sem limites) ---');

    // 1. Buscar noivas
    const { data: brides } = await supabase.from('brides').select('id, name, contract_value, balance');

    // 2. Buscar TODOS os pagamentos (usando limit grande)
    const { data: payments } = await supabase.from('payments').select('*').range(0, 5000);
